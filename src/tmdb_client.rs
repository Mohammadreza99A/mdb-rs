...
use crate::models::{
    movie::movie::Movie,
    search::{movie_search::MovieSearch, tv_search::TvSearch},
    tv::tv::Tv,
};
...
impl TmdbClient {
    ...
    pub fn get_request_url(
        ...
        return if query.is_some() && page.is_some() {
            format!(
    ...
    fn create_table(width: Option<u16>) -> Table {
    ...
    pub fn print_as_table(&self, json_obj: JsonValue) {
        let mut table = TmdbClient::create_table(
            Option::Some((term_size::dimensions().unwrap().0) as u16)
        );
        ...
        for (key, value) in json_obj.as_object().unwrap() {
            ...
            if value.is_array() {
                ...
                for elem in value.as_array() {
                    for obj in elem {
                        let mut obj_table = TmdbClient::create_table(Option::None);
        ...
        for (key, tabs) in vec_tables_map {
            let mut inner_table = TmdbClient::create_table(Option::None);
